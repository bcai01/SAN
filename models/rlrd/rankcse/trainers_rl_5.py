...
class CLTrainer(Trainer):
    ...
    def evaluate(
        ...
        def batcher(params, batch):
            ...
            sentences = [ \
                '''This sentence : " ''' + s + f''' " means {mask_token} .''' \
                for s in sentences \
                ]
    ...
    def train(self, model_path: Optional[str] = None, trial: Union["optuna.Trial", Dict[str, Any]] = None):
        ...
        if self.args.second_teacher_name_or_path is None:
            ...
            # RL
            assert "bert" in self.args.model_name_or_path or "roberta" in self.args.model_name_or_path
            assert "base" in self.args.model_name_or_path or "large" in self.args.model_name_or_path
            
            is_roberta = 'roberta' in self.args.model_name_or_path
            is_bert = "bert" in self.args.model_name_or_path and not 'roberta' in self.args.model_name_or_path
            is_base = "base" in self.args.model_name_or_path
            if is_bert and is_base:
            ...
            elif is_bert and not is_base:
                policy_model1 = PolicyNet(2, 1024, self.args.device).to(self.args.device)
                Critic_model1 = Critic(128, 2, 1024).to(self.args.device)
                policy_model2 = PolicyNet(2, 1024, self.args.device).to(self.args.device)
                Critic_model2 = Critic(128, 2, 1024).to(self.args.device)
            elif is_roberta and is_base:
                policy_model1 = PolicyNet(2, 768, self.args.device).to(self.args.device)
                Critic_model1 = Critic(128, 2, 768).to(self.args.device)
                policy_model2 = PolicyNet(2, 768, self.args.device).to(self.args.device)
                Critic_model2 = Critic(128, 2, 768).to(self.args.device)
            elif is_roberta and not is_base:
                policy_model1 = PolicyNet(2, 1024, self.args.device).to(self.args.device)
                Critic_model1 = Critic(128, 2, 1024).to(self.args.device)
                policy_model2 = PolicyNet(2, 1024, self.args.device).to(self.args.device)
                Critic_model2 = Critic(128, 2, 1024).to(self.args.device)
            else:
                raise NotImplementedError("RL training is only supported for BERT models.")
        ...
        TARGET_UPDATE = 100
        step_counter = 0  # ��ʼ�����������
        PSEUDO_EPISODE_LENGTH = 125 # ����α�غϵĳ���
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                with torch.no_grad():
                    ...
                    if teacher is not None:
                        ...
                        if num_sent >= 4 :
                            teacher_z1_z4_sim_pred = cos(z1T.unsqueeze(1), z4T.unsqueeze(0)) / self.args.z1_z4_tau2
                        ...
                        if "rank" in self.args.first_teacher_name_or_path:
                            ...
                            if num_sent >=4:
                                first_teacher_z4 = first_teacher_vecs[:,3]
                        else:
                            ...
                            if num_sent >=4:
                                first_teacher_z4 = embeddings1[:,3]
                        ...
                        if num_sent >= 4:
                            second_teacher_z4 = embeddings2[:,3]
                        # if self.args.fp16:
                        ...
                        if num_sent >=4:
                            first_teacher_z1_z4_sim = cos(first_teacher_z1.unsqueeze(1), first_teacher_z4.unsqueeze(0)) / self.args.z1_z4_tau2
                            second_teacher_z1_z4_sim = cos(second_teacher_z1.unsqueeze(1), second_teacher_z4.unsqueeze(0)) / self.args.z1_z4_tau2
                    ...
                    if num_sent >=4:
                        inputs["first_teacher_z1_z4_sim_pred"] = first_teacher_z1_z4_sim
                        inputs["second_teacher_z1_z4_sim_pred"] = second_teacher_z1_z4_sim
        ...
        if self.args.load_best_model_at_end and self.state.best_model_checkpoint is not None:
            ...
            if isinstance(self.model, PreTrainedModel):
                self.model = self.model.from_pretrained(self.state.best_model_checkpoint, model_args=self.model_args, mask_token_id=self.tokenizer.mask_token_id)
                if not self.is_model_parallel:
    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        if num_sent >=4:
            first_teacher_z1_z4_sim = inputs["first_teacher_z1_z4_sim_pred"]
            second_teacher_z1_z4_sim = inputs["second_teacher_z1_z4_sim_pred"]
        ...
        if num_sent >= 4:
            z4 = pooler_output[:, 3]
            z1_z4_cos = cos(z1.unsqueeze(1), z4.unsqueeze(0))/self.model_args.scd_temp
            cos_sim_1 = torch.cat([cos_sim, z1_z4_cos], 1)
            loss_o = loss_fct(cos_sim_1, labels)
        else:
        ...
        def get_environment_state(sim_tensor, inputs, z1, z2, cos_sim, encoder, distillation_loss_fct):
            ...
            # student_z1_z4_sim_pred = student_z1_z4_sim_pred.clone()
            ...
            if num_sent >=4:
                first_teacher_z1_z4_sim_pred = inputs["first_teacher_z1_z4_sim_pred"]
                second_teacher_z1_z4_sim_pred = inputs["second_teacher_z1_z4_sim_pred"]
            ...
            # first_z1_z4_kd_loss = distillation_loss_fct(first_teacher_z1_z4_sim_pred.to(encoder.device), student_z1_z4_sim_pred)
            # second_z1_z4_kd_loss = distillation_loss_fct(second_teacher_z1_z4_sim_pred.to(encoder.device),student_z1_z4_sim_pred)
        ...
        with torch.no_grad():
            ...
            if sim_tensor1.shape[1] == self.args.per_device_train_batch_size:
                first_action, first_avg_probability = first_teacher_policy.take_action(first_teacher_state)
                second_action, second_avg_probability = second_teacher_policy.take_action(second_teacher_state)
                model.first_states = first_teacher_state
                model.second_states = second_teacher_state
        ...
        if first_action == 0 and second_action == 0:
            ...
            if num_sent >=4:
                teacher_top1_z1_z4_pred = (weight1 * first_teacher_z1_z4_sim) + (weight2 * second_teacher_z1_z4_sim)
            
            student_top1_sim_pred = cos_sim.clone()
            kd_loss = distillation_loss_fct(teacher_top1_sim_pred.to(encoder.device), student_top1_sim_pred)

            if num_sent >= 4 and self.model_args.use_ps_dist_loss:
                z1_z4_cos = cos(z1.unsqueeze(1), z4.unsqueeze(0))/self.model_args.scd_temp
                kd_ps_loss = distillation_loss_fct(teacher_top1_z1_z4_pred.to(encoder.device), z1_z4_cos.clone())
                # print(kd_ps_loss.item())
                kd_loss = self.model_args.ps_dist_loss_weight * kd_ps_loss + kd_loss
