...
class ListMLEWithDrop(nn.Module):
    """
    ListMLE objective for ranking distillation; maximizes the liklihood of the ground truth permutation (sorted indices of the ranking lists obtained from teacher) 
    """
    def __init__(self, tau, gamma_, min_sim=-1.0, temp=0.05):
        super(ListMLEWithDrop, self).__init__()
        self.temp_scaled_sim = Similarity(tau)
        self.gamma_ = gamma_ 
        self.eps = 1e-7
        self.min_sim = min_sim / temp
    ...
    def forward(self, teacher_top1_sim_pred, student_top1_sim_pred):
        ...
        y_true_sorted, indices = y_true_shuffled.sort(descending=True, dim=-1)
        min_sim_mask = y_true_sorted < self.min_sim
        # mask = y_true_sorted == -1
        preds_sorted_by_true = torch.gather(y_pred_shuffled, dim=1, index=indices)
        # preds_sorted_by_true[mask] = float('-inf')
        preds_sorted_by_true[min_sim_mask] = float('-inf')
        max_pred_values, _ = preds_sorted_by_true.max(dim=1, keepdim=True)
        preds_sorted_by_true_minus_max = preds_sorted_by_true - max_pred_values
        cumsums = torch.cumsum(preds_sorted_by_true_minus_max.exp().flip(dims=[1]), dim=1).flip(dims=[1])
        observation_loss = torch.log(cumsums + self.eps) - preds_sorted_by_true_minus_max
        # observation_loss[mask] = 0.0
        observation_loss[min_sim_mask] = 0.0

        return self.gamma_ * torch.mean(torch.sum(observation_loss, dim=1))


class ListMLEWithOurSamples(nn.Module):
    """
    ListMLE objective for ranking distillation; maximizes the liklihood of the ground truth permutation (sorted indices of the ranking lists obtained from teacher) 
    """
    def __init__(self, tau, gamma_):
        super(ListMLEWithOurSamples, self).__init__()
        self.temp_scaled_sim = Similarity(tau)
        self.gamma_ = gamma_ 
        self.eps = 1e-7

    def forward(self, teacher_top1_sim_pred, student_top1_sim_pred, z1_z3_cos):

        y_pred = student_top1_sim_pred 
        y_true = teacher_top1_sim_pred
        y_our_our_samples = z1_z3_cos
        y_our_our_samples.diagonal().fill_(float('-inf'))
        # print(y_our_our_samples.diagonal())

        # shuffle for randomised tie resolution
        random_indices = torch.randperm(y_pred.shape[-1])
        y_pred_shuffled = y_pred[:, random_indices]
        y_true_shuffled = y_true[:, random_indices]
        y_our_our_samples_shuffled = y_our_our_samples[:, random_indices]

        y_true_sorted, indices = y_true_shuffled.sort(descending=True, dim=-1)
        mask = y_true_sorted == -1
        
        preds_sorted_by_true = torch.gather(y_pred_shuffled, dim=1, index=indices)
        our_our_samples_sorted_by_true = torch.gather(y_our_our_samples_shuffled, dim=1, index=indices)

        preds_sorted_by_true[mask] = float('-inf')
        our_our_samples_sorted_by_true[mask] = float('-inf')

        max_pred_values, _ = preds_sorted_by_true.max(dim=1, keepdim=True)
        max_our_our_samples_values, _ = our_our_samples_sorted_by_true.max(dim=1, keepdim=True)

        preds_sorted_by_true_minus_max = preds_sorted_by_true - max_pred_values
        our_our_samples_sorted_by_true_minus_max = our_our_samples_sorted_by_true - max_our_our_samples_values

        cumsums = torch.cumsum(preds_sorted_by_true_minus_max.exp().flip(dims=[1]), dim=1).flip(dims=[1])
        cumsum_our_samoles = torch.cumsum(our_our_samples_sorted_by_true_minus_max.exp().flip(dims=[1]), dim=1).flip(dims=[1])
        # print(cumsums.shape)
        # print(cumsum_our_samoles.shape)
        cumsums = cumsums + cumsum_our_samoles
        observation_loss = torch.log(cumsums + self.eps) - preds_sorted_by_true_minus_max
        observation_loss[mask] = 0.0
...
def cl_init(cls, config):
    ...
    cls.sim_tau2 = Similarity(temp=cls.model_args.tau2)
    ...
    elif cls.model_args.distillation_loss == "listmlewithoursamples":
        cls.distillation_loss_fct = ListMLEWithOurSamples(cls.model_args.tau2, cls.model_args.gamma_)
    else:
        raise NotImplementedError

    if cls.model_args.ps_dist_distillation_loss is None:
        cls.ps_dist_distillation_loss_fct = cls.distillation_loss_fct
    elif cls.model_args.ps_dist_distillation_loss == "listmlewithdrop":
        cls.ps_dist_distillation_loss_fct = ListMLEWithDrop(cls.model_args.tau2, cls.model_args.gamma_, cls.model_args.min_sim, cls.model_args.temp)
...
def cl_forward(cls,
    ...
    # Hard negative
    if num_sent >= 3:
        z1_z3_cos = cls.sim_scd(z1.unsqueeze(1), z3.unsqueeze(0))
        loss = loss_fct(torch.cat([cos_sim, z1_z3_cos], 1), labels)
    else: 
    ...
    student_top1_sim_pred = cls.sim_tau2(z1.unsqueeze(1), z2.unsqueeze(0))
    if num_sent >= 3 and cls.model_args.distillation_loss == "listmlewithoursamples":
        kd_loss = cls.distillation_loss_fct(teacher_top1_sim_pred.to(cls.device), student_top1_sim_pred, z1_z3_cos)
    else:
        kd_loss = cls.distillation_loss_fct(teacher_top1_sim_pred.to(cls.device), student_top1_sim_pred)

    if num_sent >= 3 and cls.model_args.use_ps_dist_loss:
        kd_ps_loss = cls.ps_dist_distillation_loss_fct(teacher_z1_z3_sim_pred.to(cls.device), z1_z3_cos.clone())
        # print(kd_ps_loss.item())

        kd_loss = cls.model_args.ps_dist_loss_weight * kd_ps_loss + kd_loss
    ...
    def forward(self,
        ...
        if sent_emb:
            ...
            return cl_forward(self, self.bert,
                ...
                teacher_z1_z3_sim_pred=teacher_z1_z3_sim_pred,
            )
